...
glmnet_initial <- c("naomit","impute_knn", "month", "dow", "dummy") # <-- These are arbitrary starting values. Set these to your best recommendation
pls_initial <- c("naomit","impute_knn", "pls", "month","dow", "dummy", "interact")   # <-- These are arbitrary starting values. Set these to your best recommendation
rpart_initial <- c("naomit","impute_knn", "month","dow", "dummy") # <-- These are arbitrary starting values. Set these to your best recommendation
# maintenance point ---------------------------------------------------------------------------------------------------------------------------
# add further preprocessing choices for the new methods here

brnn_initial <- c("naomit","impute_bag", "month", "dow", "dummy", "nzv") # <-- These are arbitrary starting values. Set these to your best recommendation

qrnn_initial <- c("naomit","impute_knn", "month", "dow", "dummy")

bstTree_initial <- c("naomit","impute_bag", "month", "dow", "dummy")

svmRadial_initial <- c("naomit","impute_knn", "month", "dow", "dummy", "interact", "center", "scale")

svmLinear_initial <- c("naomit","impute_knn", "month", "dow", "dummy")
...
dynamicSteps <- function(recipe, preprocess) {
  ...
  for (s in preprocess) {
    if (s == "impute_knn") { #impute missing data using nearest neighbors.
      recipe <- step_impute_knn(recipe, all_numeric_predictors(), all_nominal_predictors(), neighbors = 5) # 5 is a reasonable guess
    } else if (s == "impute_bag") { #create bagged tree models to impute missing data.
    ...
    } else if (s == "naomit") { #remove observations (rows of data) if they contain NA or NaN values.
    ...
    } else if (s == "month") { #makes an extra month col 
      recipe <- recipes::step_date(recipe, has_type("date"), features = c("month"), ordinal = FALSE)
    } else if (s == "dow") { #makes an extra day of week col 
    ...
    } else if (s == "dummy") { #convert nominal data (e.g. character or factors) into one or more numeric binary model terms for the levels of the original data.
